    ...
    <>
      <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose?.()}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto overflow-x-hidden" hideCloseButton>
          <button
            onClick={() => onClose?.()}
            className="absolute top-4 right-4 z-50 flex items-center justify-center w-16 h-10 rounded-md backdrop-blur-md bg-white/20 dark:bg-white/20 border border-white/40 text-foreground hover:bg-white/30 hover:scale-105 transition-all duration-200 shadow-lg"
          >
            <span className="text-sm font-medium">Cancel</span>
          </button>

          <DialogHeader>
            <DialogTitle className="text-2xl">Select an Event</DialogTitle>
            <p className="text-muted-foreground">You have multiple events. Choose one to access its dashboard.</p>
          </DialogHeader>
      ...
      <AlertDialog open={deleteEventId !== null} onOpenChange={() => setDeleteEventId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete the event and all associated RSVP data.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
